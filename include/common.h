...
static inline float random_float_interval(float min, float max) {
  return min + (max-min)*random_float();
}

static inline HMM_Vec3 random_vector() {
  HMM_Vec3 random = { .X = random_float(), .Y = random_float(), .Z = random_float() };
  return random;
}

static inline HMM_Vec3 random_vector_interval(float min, float max) {
